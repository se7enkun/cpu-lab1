...
`else
    ...
    // initialize ddr with .hex file
    // reg [BL_MAX*DQ_BITS-1:0] init_data_line;
    // reg [BA_BITS - 1 : 0] init_bank;
    // reg [ROW_BITS - 1 : 0] init_row;
    // reg [COL_BITS - 1 : 0] init_col;
    // reg [BA_BITS + ROW_BITS + COL_BITS - 1 : 0] init_addr = 'h0;    // half-word address
    // integer init_fp, init_fp_status;
    // wire mig_init_done = soc_simple_tb.DUT.init_calib_complete;
    // initial begin
    //     wait(mig_init_done === 1'b1);

    //     init_fp = $fopen("../../../../soft/ddr_init.hex","r");

    //     while (!$feof(init_fp)) begin

    //         init_fp_status = $fscanf(init_fp, "%h", init_data_line);

    //         if (init_fp_status != -1) begin // Check for blank line or EOF
    //             init_bank = init_addr[BA_BITS + ROW_BITS + COL_BITS - 1 : ROW_BITS + COL_BITS];
    //             init_row  = init_addr[ROW_BITS + COL_BITS - 1 : COL_BITS];
    //             init_col  = init_addr[COL_BITS - 1 : 0];

    //             memory_write (init_bank, init_row, init_col, init_data_line);

    //             init_addr = init_addr + 'h8;    // same as byte-address + 0x10
    //         end
    //     end

    //     $fclose(init_fp);
    //     // $readmemh("../../../../soft/ddr_init.hex", memory);
    // end
