...
&emsp;&emsp;`ecall`指令用于实现系统调用。处理器执行`ecall`指令将引起例外。一般地，`ecall`的例外处理程序将`a7`寄存器的值作为系统调用号，并使用`a0`、`a1`等寄存器传递参数，比如RARS的系统调用（RARS菜单 -> Help -> Syscalls）。注意，在RISC-V中，**例外是不可屏蔽的**。
